...
function getAverage(value1, value2) {
  const b = (value1 + value2) / 2;
  return b < Number.MAX_VALUE ? b : Number.MAX_VALUE;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const b = x1 * x2 + y1 * y2;
  const c = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const angle = b / c;
...
function getSumOfDigits(num) {
  const mas = Array.from(num.toString(), Number);
  let sum = 0;
  mas.forEach((el) => {
    sum += el;
  });
